...
## 2026-09-10 owner 결정: 미검증 updater 시나리오는 보류

관리자 인증 취소/복구, 프로세스 소유권, 실제 macOS 13 실행 검증은 미검증이지만
**활성 작업에서 제외**한다. 격리 QA 페어/macOS 13 기기 준비 비용 대비 실익이 낮다는
owner 판단이며, 릴리스 노트·acceptance 문서의 공시된 known limitation으로
유지한다(`minimumSystemVersion: 13.0` 선언은 유지). 사용자 이슈나 PR로 실제
사례가 보고되면 그때 대응한다. 아래 문서의 "남은 차단 조건"에서 이 시나리오들을
다시 활성 작업으로 부활시키지 않는다.
