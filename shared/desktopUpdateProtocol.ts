...
export type DesktopUpdateCommand =
  | { action: 'status' | 'check' }
  | { action: 'download' | 'restart'; targetId: string }
  | { action: 'setAutomatic'; automatic: boolean };

export const DESKTOP_UPDATE_PHASES = ['disabled', 'idle', 'checking', 'available', 'downloading', 'verifying', 'ready', 'deferred', 'error', 'applying', 'restarting', 'recovery'] as const;
export type DesktopUpdateSnapshot = {
  ...
  /** Native-owned candidate identity; never a URL, path or install authority. */
...
export const isDesktopUpdateTargetId = (value: unknown): value is string => typeof value === 'string' && /^[a-f0-9]{64}$/u.test(value);
...
export function isDesktopUpdateCommand(value: unknown): value is DesktopUpdateCommand {
  ...
  if (value.action === 'download' || value.action === 'restart') return Object.keys(value).length === 2 && isDesktopUpdateTargetId(value.targetId);
  return Object.keys(value).length === 1 && (value.action === 'status' || value.action === 'check');
}

export function isDesktopUpdateSnapshot(value: unknown): value is DesktopUpdateSnapshot {
  const keys = ['protocolVersion', 'phase', 'automatic', 'productVersion', 'desktopVersion', 'targetProductVersion', 'targetDesktopVersion', 'targetId', 'discoveryIncomplete', 'reason', 'installationAvailable', 'downloadedBytes', 'totalBytes', 'notes'];
  if (!record(value) || Object.keys(value).length !== keys.length || !keys.every((key) => Object.prototype.hasOwnProperty.call(value, key))
    ...
    || typeof value.installationAvailable !== 'boolean'
    || (value.targetId !== null && !isDesktopUpdateTargetId(value.targetId))) return false;
  if (['available', 'downloading', 'verifying', 'ready'].includes(String(value.phase))
    && (value.targetId === null || !value.targetProductVersion || !value.targetDesktopVersion)) return false;
