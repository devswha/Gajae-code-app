...
test('restart appears only for native available+ready, with OS prompt explanation and no credential inputs', async () => {
  const commands: DesktopUpdateCommand[] = [];
  ...
  await act(async () => { window.dispatchEvent(new Event(DESKTOP_UPDATE_BRIDGE_EVENT)); });
  const restart = screen.getByRole('button', { name: english.desktopUpdate.restart });
  const description = restart.getAttribute('aria-describedby');
  assert.equal(document.getElementById(description ?? '')?.textContent, english.desktopUpdate.osPrompt);
...
test('English and Korean expose translated controls, progress, reasons and OS prompt instructions', async () => {
  ...
  assert.ok(screen.getByRole('button', { name: korean.desktopUpdate.restart }));
  assert.ok(screen.getByText(korean.desktopUpdate.osPrompt));
  await act(async () => { await view.i18n.changeLanguage('en'); });
  assert.ok(screen.getByRole('button', { name: english.desktopUpdate.restart }));
